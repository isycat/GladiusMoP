...
if not GladiusMoP then
  DEFAULT_CHAT_FRAME:AddMessage(format("Module %s requires GladiusMoP", "Announcements"))
end
...
local Announcements = GladiusMoP:NewModule("Announcements", false, false, {
   announcements = {
      drinks = true,
      enemies = true,
      health = true,
...
function Announcements:OnEnable()
   ...
   self:RegisterMessage("GLADIUSMOP_SPEC_UPDATE")
...
function Announcements:UNIT_NAME_UPDATE(event, unit)
   ...
   if (not GladiusMoP.db.announcements.enemies or not UnitName(unit)) then return end
...
function Announcements:GLADIUSMOP_SPEC_UPDATE(unit, event)
   if (not strfind(unit, "arena") or strfind(unit, "pet") or not GladiusMoP.db.announcements.spec) then return end
   self:Send(string.format(L["SPEC DETECTED: %s (%s)"], UnitName(unit), GladiusMoP.buttons[unit].spec), 2, unit)
end

function Announcements:UNIT_HEALTH(event, unit)
   if (not strfind(unit, "arena") or strfind(unit, "pet") or not GladiusMoP.db.announcements.health) then return end
   
   local healthPercent = math.floor((UnitHealth(unit) / UnitHealthMax(unit)) * 100)
   if (healthPercent < GladiusMoP.db.announcements.healthThreshold) then
...
function Announcements:UNIT_AURA(event, unit)
   if (not strfind(unit, "arena") or strfind(unit, "pet") or not GladiusMoP.db.announcements.drinks) then return end
...
function Announcements:UNIT_SPELLCAST_START(event, unit, spell, rank)
   if (not strfind(unit, "arena") or strfind(unit, "pet") or not GladiusMoP.db.announcements.resurrect) then return end
...
function Announcements:Send(msg, throttle, unit)
   ...
   local dest = GladiusMoP.db.announcements.dest
   ...
   if (dest == "self") then
      DEFAULT_CHAT_FRAME:AddMessage("|cff33ff99GladiusMoP|r: " .. msg)
   end
	
	-- change destination to party if not raid leader/officer.
	if(dest == "rw" and not UnitIsGroupLeader(unit) and not UnitIsGroupAssistant(unit) and GetNumGroupMembers() > 0) then
		dest = "party"
	end
...
local function getOption(info)
   return GladiusMoP.dbi.profile.announcements[info[#info]]
end

local function setOption(info, value)
   GladiusMoP.dbi.profile.announcements[info[#info]] = value
end
            ...
            options = {
               ...
               args = {
                  ...
                  healthThreshold = {
                     ...
                     disabled=function() return not GladiusMoP.db.announcements.health end,
                     min=1,
                     max=100,
                     step=1,
                     order=10,
