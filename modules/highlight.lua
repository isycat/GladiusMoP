...
if not GladiusMoP then
  DEFAULT_CHAT_FRAME:AddMessage(format("Module %s requires GladiusMoP", "Highlight"))
end
...
local Highlight = GladiusMoP:NewModule("Highlight", false, false, {
   highlightHover = true,
...
function Highlight:UNIT_TARGET(event, unit)
   local unit = unit or ""
   if (unit ~= "" and ((not IsInRaid()) or strfind(unit, "pet") or (not strfind(unit, "raid") and not strfind(unit, "party")))) then return end  
   ...
   for arenaUnit, frame in pairs(self.frame) do
      ...
      if (targetGUID and UnitGUID(arenaUnit) == targetGUID and unit ~= "") then 
         -- main assist
         if (GladiusMoP.db.highlightAssist and GetPartyAssignment("MAINASSIST", unit) == 1) then            
            if (frame.priority < GladiusMoP.db.highlightTargetPriority) then
               frame.priority = GladiusMoP.db.highlightTargetPriority
               frame:SetBackdropBorderColor(GladiusMoP.db.highlightTargetColor.r, GladiusMoP.db.highlightTargetColor.g, GladiusMoP.db.highlightTargetColor.b, GladiusMoP.db.highlightTargetColor.a)            
            end
         end
         
         -- raid target icon
         local icon = GetRaidTargetIndex(unit)
         if (icon and GladiusMoP.db["highlightRaidIcon" .. icon]) then            
            if (frame.priority < GladiusMoP.db["highlightRaidIcon" .. icon .. "Priority"]) then
               frame.priority = GladiusMoP.db["highlightRaidIcon" .. icon .. "Priority"]
               frame:SetBackdropBorderColor(GladiusMoP.db["highlightRaidIcon" .. icon .. "Color"].r, GladiusMoP.db["highlightRaidIcon" .. icon .. "Color"].g, 
                  GladiusMoP.db["highlightRaidIcon" .. icon .. "Color"].b, GladiusMoP.db["highlightRaidIcon" .. icon .. "Color"].a)            
            end
         end
      end
      ...
      if (focusGUID and UnitGUID(arenaUnit) == focusGUID) then
         if (frame.priority < GladiusMoP.db.highlightFocusPriority) then
            frame.priority = GladiusMoP.db.highlightFocusPriority
            frame:SetBackdropBorderColor(GladiusMoP.db.highlightFocusColor.r, GladiusMoP.db.highlightFocusColor.g, GladiusMoP.db.highlightFocusColor.b, GladiusMoP.db.highlightFocusColor.a)            
         end
      end
...
function Highlight:CreateFrame(unit)
   ...
   -- create frame
   self.frame[unit] = CreateFrame("Frame", "GladiusMoP" .. self.name .. unit, button)
...
function Highlight:Update(unit) 
   ...
   self.frame[unit]:SetPoint("TOPLEFT", GladiusMoP.buttons[unit], "TOPLEFT", left - 3, top + 3)
   
   self.frame[unit]:SetWidth(GladiusMoP.buttons[unit]:GetWidth() + abs(left) + abs(right) + 3)
   self.frame[unit]:SetHeight(GladiusMoP.buttons[unit]:GetHeight() + abs(bottom) + abs(top) + 3)
   ...
   if (GladiusMoP.db.highlightHover) then      
      -- set scripts
      button:SetScript("OnEnter", function(f, motion)
         if (motion and f:GetAlpha() > 0) then
            for _, m in pairs(GladiusMoP.modules) do
               if (m:IsEnabled() and m.frame and m.frame[unit].highlight) then
                  -- set color
                  m.frame[unit].highlight:SetVertexColor(GladiusMoP.db.highlightHoverColor.r, GladiusMoP.db.highlightHoverColor.g,
                     GladiusMoP.db.highlightHoverColor.b, GladiusMoP.db.highlightHoverColor.a)
