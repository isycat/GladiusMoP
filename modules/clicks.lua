...
if not GladiusMoP then
  DEFAULT_CHAT_FRAME:AddMessage(format("Module %s requires GladiusMoP", "Clicks"))
end
...
local Clicks = GladiusMoP:NewModule("Clicks", false, false, {
   clickAttributes = {
...
function Clicks:OnDisable()
   ...
   for _,t in pairs(self.secureFrames) do
      for frame,_ in pairs(t) do
         for _,attr in pairs(GladiusMoP.dbi.profile.clickAttributes) do
...
function Clicks:GetSecureFrames(unit)
   ...
   self:RegisterSecureFrame(unit, GladiusMoP.buttons[unit].secure)
   
   -- Find secure frames in other modules
   for m, _ in pairs(GladiusMoP.modules) do
      local frame = GladiusMoP:GetParent(unit, m)
...
local function getOption(info)
   ...
   return GladiusMoP.dbi.profile.clickAttributes[key][info[#info]]
end

local function setOption(info, value)
   local key = info[#info - 2]
   GladiusMoP.dbi.profile.clickAttributes[key][info[#info]] = value
...
function Clicks:GetOptions()
               ...
               args = {
                  ...
                  add = {
                     ...
                     func=function()
                        ...
                        if (not GladiusMoP.db.clickAttributes[attr]) then                           
                           -- add to db
                           GladiusMoP.db.clickAttributes[attr] = {
                           ...
                           -- add to options
                           GladiusMoP.options.args[self.name].args.attributeList.args[attr] = self:GetAttributeOptionTable(attr, order)
   ...
   -- attributes
   local order = 1
   for attr, _ in pairs(GladiusMoP.dbi.profile.clickAttributes) do 
   ...
   return {  
      ...
      args = {
         delete = {
            ...
            func=function()
               -- remove from db
               GladiusMoP.db.clickAttributes[attribute] = nil
               
               -- remove from options
               GladiusMoP.options.args[self.name].args.attributeList.args[attribute] = nil           
